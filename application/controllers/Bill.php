                ...
                switch ($command) {
                    case "ลงทะเบียน":
                        if (count($msgqarr) >= 3) {
                            ...
                            $replymsg = "กรุณาระบุชื่อของท่านด้วยเช่น 'ลงทะเบียน cf-order เจ้าของร้าน'";
                        }
                    ...
                    case "ค้นหา":
                        ...
                        if (count($orderhistory) > 0) {
                            ...
                            foreach ($orderhistory as $index => $item) {
                                ...
                                $msg .= "$no. http://cf-order.com/$item->token \n    $paidtime\n";
                                $no++;
                            }
                        }
                        ...
                        foreach ($merchants as $item) {
                            ...
                            for ($i = count($billcolums); $i < 3; $i++) {
                                array_push($billcolums, array("type" => "message",
                                    "label" => "-",
                                    "text" => "สามารถสร้างบิลเพิ่มได้ที่ http://cf-order.com/account/$item->token/dashboard"));
                            }
                        ...
                        array_push($colums, array(
                            ...
                            'actions' => array(
                                array(
                                    ...
                                    "uri" => "http://cf-order.com/pro/$item->token/$uid"
                                )
                            )
                        ));
                    }
            ...
            if (count($msgqarr) >= 2) {
                switch ($command) {
                    case "บิล":
                        ...
                        $replymsg = "ลูกค้าสามารถซื้อสินค้าได้ที่ลิงค์นี้ http://cf-order.com/$merchant->token";
                        break;
                    default:
                        return false;
                        break;
    ...
    public function getmessagepostback($postbackdata)
    {
        ...
        if ($msg[0] == "getbilltoken") {
            ...
            if ($data["sqlmerchant"]->num_rows() > 0) {
                ...
                return $messages = [
                    'type' => 'text',
                    'text' => "http://cf-order.com/$billtoken"
                ];
            }
        }
        ...
        if ($msg[0] == "getbilltokennogen") {
            ...
            return $messages = [
                'type' => 'text',
                'text' => "http://cf-order.com/$billtoken->token"
            ];
